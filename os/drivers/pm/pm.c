 ...
 *   PMIOC_TIMER_CREATE - to create a wakeup timer
 *   PMIOC_TIMER_SET - to set a wakeup timer
 *   PMIOC_TIMER_STOP - to stop a specific wakeup timer
 *   PMIOC_TIMER_DELETE - to delete a specific wakeup timer and make it available for other apps
 ...
 *   the command. 
 *   for LOCK, arg should be a long unsigned type.
 *   for TIMER_CREATE, arg should be a bool value.
 *   for TIMER_SET, arg should be a pointer to struct pm_timer_header type.
 *   for TIMER_STOP and TIMER_DELETE, arg should be a int (id of timer).
...
static int pm_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	switch (cmd) {
        ...
        case PMIOC_UNLOCK:
            	...
            	ret = OK;
            	break;
	case PMIOC_TIMER_CREATE:
		ret = pm_timer_create((bool)arg);
		break;
	case PMIOC_TIMER_SET:
		ret = pm_timer_set(((struct pm_timer_header *)arg)->id, ((struct pm_timer_header *)arg)->timer_interval);
		break;
	case PMIOC_TIMER_STOP:
		ret = pm_timer_stop((int)arg);
		break;
	case PMIOC_TIMER_DELETE:
		ret = pm_timer_delete((int)arg);
		break;
